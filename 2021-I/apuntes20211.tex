...
\begin{tikzpicture}[thick,scale=1, every node/.style={scale=0.9}]
    ...
    \foreach \y in {-2,-1,0,1,2}
        \draw (1pt, \y cm) -- (-1pt, \y cm);
    \draw[AZUL,dotted,thin] (-3,-3) -- (3,3) node[] at (-1,-2) {$E_{\lambda_1}$};
    \draw[ROJO,dotted,thin] (3,-3) -- (-3,3) node[] at (1,-2) {$E_{\lambda_2}$};
    \draw[darkgray,->] (0,0) -- (0.5,2) node[] at (0.5,2.5) {$\ket{v}$};
    \draw[AZUL,->] (0,0) -- (0.707,0.707) node[] at (1.3,0.5) {$\ket{g_1}$};
    \draw[ROJO,->] (0,0) -- (-0.707,0.707) node[] at (-1.3,0.5) {$\ket{g_2}$};
...
\item Sean $V$ un espacio de dimensión finita con producto escalar, $T:V\to V$ un operador lineal y $\Lambda=\{\lambda_1,\lambda_2,... \ ,\lambda_k\}$ el espectro de $T$. Demuestra que $T$ es diagonalizable si y sólo si $V = E_{\lambda_i} \oplus (E_{\lambda_i})^\perp$ para toda $\lambda_i\in\Lambda$.
...
Para determinar si un operador tiene una base ortogonal de eigenvectores y, por lo tanto, si se puede descomponer espectralmente, una herramienta muy útil es entender cómo se relaciona con su operador adjunto. En un primer acercamiento, podemos analizar cómo se relacionan los eigenvectores y eigenvalores de uno y otro.

\begin{lema}\empty
    Sea $T$ un operador lineal en un espacio vectorial de dimensión finita con producto escalar $V$. Si $T$ tiene al menos un eigenvector con eigenvalor $\lambda$, entonces $T^*$ tiene un eigenvector con eigenvalor $\overline{\lambda}$.
...
([T]_\gamma)},$$ por lo que $[T^*]$ también es diagonal. Ya que las matrices diagonales conmutan, se sigue que $[T]_\gamma[T^*]_\gamma=[T^*]_\gamma[T]_\gamma$. En particular, como este isomorfismo es compatible con la multiplicación de matrices y la composición de transformaciones lineales, concluimos que $TT^*=T^*T$.

\begin{tcolorbox}
    \underline{Def.} Sea $V$ un espacio vectorial con producto escalar. Decimos que un operador lineal $T:V\to V$ es \emph{normal} si $TT^*=T^*T$.

    \vspace{3mm}
    \underline{Def.} Sea $A\in M_{n\times n}(K)$ con $K=\mathbb{R}$ ó $K=\mathbb{C}$. Decimos que $A$ es una matriz \emph{normal}\footnote{Nótese que del Teorema 14.3 se sigue que, si $V$ es de dimensión finita, entonces $T$ es un operador normal si y sólo si $[T^*]$ es una matriz normal.} si $AA^*=A^*A$.
\end{tcolorbox}

Nótese del párrafo anterior que, bajo las condiciones que impusimos al operador lineal $T$, tenemos que $T$ y $T^*$ tienen los mismos eigenvectores \textemdash aunque no necesariamente con los mismos eigenvalores correspondientes\footnote{En particular, en este caso $T$ y $T^*$ son simultáneamente diagonalizables.}. Ahora veremos cómo se relacionan los eigenvalores bajo estas hipótesis.

\begin{teorema} {14.4}
    Sean $V$ un espacio vectorial de dimensión finita $n$ con producto escalar y $T$ un operador lineal normal en $V$. Entonces, para todo $\ket{v}\in V$ tal que $T(\ket{v})=\lambda \ket{v}$, tenemos que $T^*(\ket{v})=\overline{\lambda}\ket{(v)}$.

\begin{proof}
    Ya que $T$ es normal, tenemos que $TT^*=T^*T$. Sea $\ket{v}\in V$ tal que $T(\ket{v})=\lambda \ket{v}$. Entonces, se sigue que COMPLETAR
\end{proof}
\end{teorema}


\vspace{3mm}
Parece que ya estamos muy cerca de nuestra meta; sin embargo, observemos lo siguiente: si $T$ es un operador normal en un espacio vectorial $(V,K)$ de dimensión finita con producto escalar y $\ket{g_i},\ket{g_j}$ son eigenvectores de $T$ con eigenvalores $\lambda_i, \lambda_j$ tales que $\lambda_i\neq \lambda_j$, entonces por un lado tenemos que $$\braket{g_i|TT^*(g_j)}=\braket{T^*(g_i)|(T^*(g_j))}=\braket{\overline{\lambda_i}g_i|\overline{\lambda_j}g_j}=\lambda_i \overline{\lambda_j}\braket{g_i|g_j},$$ mientras que, por otro lado, $$\braket{g_i|TT^*(g_j)}=\braket{g_i|T^*T(g_j)}=\braket{T(g_i)|T(g_j)}=\braket{\lambda_i g_i|\lambda_jg_j}=\overline{\lambda_i}\lambda_j\braket{g_i|g_j}=\overline{(\lambda_i \overline{\lambda_j})}\braket{g_i|g_j}.$$ Por lo tanto, llegamos a la igualdad $$(\lambda_i \overline{\lambda_j} - \overline{(\lambda_i \overline{\lambda_j})})\braket{g_i|g_j}=0.$$ Observemos que si $K=\mathbb{C}$, entonces forzosamente $\ket{g_i}$ y $\ket{g_j}$ son ortogonales, de donde se sigue que $E_{\lambda_i}^\perp=\oplus_{j\neq i}E_{\lambda_j}$ para todo eigenvalor $\lambda_i$ de $T$ y, por ende, que $T$ se puede descomponer espectralmente.
...
\begin{enumerate}
    \item Demuestra que las matrices diagonales con entradas en un campo $K$ conmutan.
\end{enumerate}
